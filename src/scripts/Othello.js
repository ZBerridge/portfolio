export default {
    othelloGame() {
        ...
        function initPlayerTurns(){
            let turns = document.querySelectorAll(".player-turn"), i;
            for(i = 0; i < turns.length; i++) {
                let x = turns[i].getAttribute("data-x");
                let y = turns[i].getAttribute("data-y");
                turns[i].addEventListener("click",  function(e){
                    e = e || window.event;
                    let src = e.target;
                    let eventpath = e.composedPath()
                    let x = eventpath[0].dataset.x;
                    let y = eventpath[0].dataset.y;
                    playerTurn(x,y);
                });
            }
        }
        ...
        function drawScoreBoard() {
            //debugger;
            let htmlString = "<div id='scoreKeep'><div>";
        ...
        function checkForMoves() {
            ...
            if (hasValid == 0) {
                let outputString = capitalizeFirstLetter(playerColor);
